...
def _collect_module_string_constants(module: ast.Module) -> dict[str, str]:
    """Collect simple module-level string constants like: SOME_LABEL = "..."."""
    ...
    for node in module.body:
        if isinstance(node, ast.Assign) and len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
            ...
            if isinstance(val, ast.Constant) and isinstance(val.value, str):
                constants[name] = val.value
    return constants


def _extract_str(node: ast.AST, constants: dict[str, str] | None = None) -> str | None:
    """Best-effort extract a Python string from an AST node.

    Supports:
    - Literal strings: "xxx"
    - gettext wrappers: _( "xxx" ), gettext_lazy("xxx"), etc.
    - Simple module constants: LABEL_X = "xxx"; verbose_name=LABEL_X
    """
    if isinstance(node, ast.Constant) and isinstance(node.value, str):
        return node.value

    # e.g. verbose_name=LABEL_X
    if isinstance(node, ast.Name) and constants and node.id in constants:
        return constants[node.id]

    # e.g. verbose_name=_("xxx") or gettext_lazy("xxx")
    if isinstance(node, ast.Call) and node.args:
        ...
        if func_name in {
            ...
            return _extract_str(node.args[0], constants=constants)
...
def build_fields_for_export() -> list[tuple[str, str]]:
    ...
    Notes:
    - This function also understands common i18n wrappers like gettext_lazy() / _().
    - If verbose_name is provided via a simple module-level string constant,
      it will be resolved as well.
    ...
    for node in patient_class.body:
        ...
        for kw in value.keywords:
            if kw.arg == "verbose_name":
                verbose_name = _extract_str(kw.value, constants=constants)
                if verbose_name:
                    break

        # If not set, use the first positional arg if it resolves to a string
        if verbose_name is None and value.args:
            verbose_name = _extract_str(value.args[0], constants=constants)
